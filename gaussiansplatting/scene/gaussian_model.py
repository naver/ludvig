...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self._xyz_backup = torch.empty(0)
        self._features_dc_backup = torch.empty(0)
        self._features_rest_backup = torch.empty(0)
        self._scaling_backup = torch.empty(0)
        self._rotation_backup = torch.empty(0)
        self._opacity_backup = torch.empty(0)
        self.max_radii2D_backup = torch.empty(0)
        self.xyz_gradient_accum = torch.empty(0)
        self.denom = torch.empty(0)
        self.opacity_scaling = torch.empty(0).cuda()
        self.optimizer = None
    ...
    def recover_points(self):
        if not len(self._xyz_backup):
            return
        self._xyz = nn.Parameter(self._xyz_backup)
        self._features_dc = nn.Parameter(self._features_dc_backup)
        self._features_rest = nn.Parameter(self._features_rest_backup)
        self._opacity = nn.Parameter(self._opacity_backup)
        self._scaling = nn.Parameter(self._scaling_backup)
        self._rotation = nn.Parameter(self._rotation_backup)
        self.max_radii2D = self.max_radii2D_backup


    def prune_points_noopt(self, perm, backup=False):
        if backup:
            self._xyz_backup = self._xyz.data
            self._features_dc_backup = self._features_dc.data
            self._features_rest_backup = self._features_rest.data
            self._opacity_backup = self._opacity.data
            self._scaling_backup = self._scaling.data
            self._rotation_backup = self._rotation.data
            self.max_radii2D_backup = self.max_radii2D
        ...
        self.max_radii2D = self.max_radii2D[perm]
