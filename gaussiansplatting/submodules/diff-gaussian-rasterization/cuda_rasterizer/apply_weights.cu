...
// Forward method for converting scale and radiarotation properties of each
...
template <int C>
__global__ void preprocessCUDA_apply_weights(
  ...
  float3 cov = computeCov2D_apply_weights(p_orig, focal_x, focal_y, tan_fovx,
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA_apply_weights(
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      for (int ch = 0; ch < CHANNELS; ch++) {
        atomicAdd(weights + (collected_id[j] * CHANNELS + ch), C[ch] * alpha * T);
      ...
      // int _j = (pix_id % W) / 14;
      // int new_id = _i*14 + _j;
      // int channels = (H/14) * (W/14);
      
      // int _i = (pix_id / W) / (H/64); // between 0 and 63
      // int _j = (pix_id % W) / (W/64); // between 0 and 63
      // int new_id = _i*64 + _j; // between 0 and 64*64-1 
      // int channels = 64 * 64;
      // weights[collected_id[j] * channels + new_id] += alpha*T;
      // // atomicAdd(weights + collected_id[j] * channels + new_id, alpha*T);
...
void APPLY_WEIGHTS::render(const dim3 grid, dim3 block, const uint2 *ranges,
  ...
  if (num_channels == 1) {
    ...
    renderCUDA_apply_weights<10><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 20) {
    renderCUDA_apply_weights<20><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 30) {
    renderCUDA_apply_weights<30><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 40) {
    renderCUDA_apply_weights<40><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 50) {
    renderCUDA_apply_weights<50><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 100) {
    renderCUDA_apply_weights<100><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 200) {
    renderCUDA_apply_weights<200><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 256) {
    renderCUDA_apply_weights<256><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else if (num_channels == 512) {
    renderCUDA_apply_weights<512><<<grid, block>>>(
        ranges, point_list, W, H, means2D, weights, conic_opacity, final_T,
        n_contrib, bg_color, image_weights, cnt);
  } else {
